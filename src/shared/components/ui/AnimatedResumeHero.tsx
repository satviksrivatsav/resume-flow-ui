...
export const AnimatedResumeHero = ({ className }: { className?: string }) => {
  ...
  const Cursor = ({ active }: { active: boolean }) => (
    <motion.span
      ...
      className={cn(
        'inline-block w-0.5 h-[1em] bg-white ml-0.5 translate-y-0.5',
        ...
        <div className="space-y-6 md:space-y-8 relative z-10 text-left">
          <div className="space-y-1">
            <h3 className="text-2xl md:text-3xl font-bold text-white tracking-tight h-8">
              {displayedTexts[0]}
              <Cursor active={currentBlock === 0} />
            </h3>
            <p className="text-zinc-200 font-medium text-base md:text-lg h-6">
              {displayedTexts[1]}
              <Cursor active={currentBlock === 1} />
            </p>
          </div>

          <div className="space-y-4">
            <div className="text-[10px] md:text-sm font-semibold text-white/40 uppercase tracking-widest">
              About
            </div>
            <div className="text-zinc-300 text-xs md:text-sm font-mono leading-relaxed min-h-[40px]">
              {displayedTexts[2]}
              <Cursor active={currentBlock === 2} />
            </div>
          </div>
          ...
          <div className="pt-2 space-y-3">
            ...
            <div className="flex gap-2 flex-wrap text-xs md:text-sm font-mono text-zinc-400">
              <span className="bg-white/5 px-2 py-1 rounded inline-block">
                {displayedTexts[6]}
                <Cursor active={currentBlock === 6} />
              </span>
              <span className="bg-white/5 px-2 py-1 rounded inline-block">
                {displayedTexts[7]}
                <Cursor active={currentBlock === 7} />
              </span>
              <span className="bg-white/5 px-2 py-1 rounded inline-block">
                {displayedTexts[8]}
                <Cursor active={currentBlock === 8} />
              </span>
              <span className="bg-white/5 px-2 py-1 rounded inline-block">
                {displayedTexts[9]}
                <Cursor active={currentBlock === 9} />
              </span>
              <span className="bg-white/5 px-2 py-1 rounded inline-block">
                {displayedTexts[10]}
                <Cursor active={currentBlock === 10} />
              </span>
              <span className="bg-white/5 px-2 py-1 rounded inline-block">
                {displayedTexts[11]}
                <Cursor active={currentBlock === 11} />
              </span>
              <span className="bg-white/5 px-2 py-1 rounded inline-block">
                {displayedTexts[12]}
                <Cursor active={currentBlock === 12} />
              </span>
            </div>
          </div>
        </div>
