...
export function EmailChangeModal({ isOpen, onClose, onConfirm, newEmail }: EmailChangeModalProps) {
  return (
      ...
      <AlertDialogContent className="sm:max-w-[440px] bg-background backdrop-blur-xl border-border/50 rounded-[2.5rem] p-8 shadow-2xl flex flex-col items-center text-center">
        ...
        <AlertDialogFooter className="mt-6 flex sm:flex-row flex-col items-center sm:justify-center sm:space-x-0 gap-3 w-full">
          ...
          <AlertDialogAction
            type="button"
            onClick={onConfirm}
            className="rounded-full h-11 px-10 font-bold bg-primary hover:bg-primary/90 transition-all active:scale-[0.98] w-full sm:w-auto"
          >
            Continue
          </AlertDialogAction>
        </AlertDialogFooter>
